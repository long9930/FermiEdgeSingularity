...
char StructureName[50] = "FES2D";

double n2D = 1e12; // in cm^(-2)
...
int main()
{
	...
	cout << effmE/m0 << "\t" << effmH/m0 << endl;

	double Egeff0;
	Egeff0 = eigEe[0] - eigEhhv[0];
	//cout << Egeff/echarge << endl;	
	...
	double * VsPPq1_array = new double [Nq];
	double * VsPPq4_array = new double [Nq];
	double * VsTFq_array = new double [Nq];
	...
	for (int nq = 0; nq < Nq; nq++)
	{
		...
		Vqform = 1.0;
		VsPPq1_array[nq] = Vqcoef*Vqform*(1.0/kappa+Cqcoef*pow(qq,2.0))/(1.0+qq/kappa+Cqcoef*pow(qq,3.0));		
	}	
	}
	...
	for (int nq = 0; nq < Nq; nq++)
	{
		...
		Cqcoef = (Cq/4.0)*pow(hbar/(2*effmR),2.0)/wplsqcoef;
		Vqform = 1.0;
		VsPPq4_array[nq] = Vqcoef*Vqform*(1.0/kappa+Cqcoef*pow(qq,2.0))/(1.0+qq/kappa+Cqcoef*pow(qq,3.0));		
	}
	}
	...
	for (int nq = 0; nq < Nq; nq++)
	{
		...
		double Vqform;
		Vqform = 1.0;
		VsTFq_array[nq] = Vqcoef*Vqform*1.0/(kappa+qq);		
	}	
	...
	const int Nsavef1 = 200;
	char fq_name[Nsavef1];
	char fVsq_name[Nsavef1];
	char fVsPPq1_name[Nsavef1];
	char fVsPPq4_name[Nsavef1];
	char fVsTFq_name[Nsavef1];
	char fPolEq_name[Nsavef1];
	char fPolHq_name[Nsavef1];

	char prefix1[100];
	sprintf_s(prefix1, 100, "%s_n2D%3.2e_Tl%.0f_Te%.0f_",StructureName,n2D*1e-4,T_lattice,T_electron);
	sprintf_s(fq_name, Nsavef1, "%sq.txt", prefix1);
	sprintf_s(fVsq_name, Nsavef1, "%sVsq.txt", prefix1);
	sprintf_s(fVsPPq1_name, Nsavef1, "%sVsPPq1.txt", prefix1);
	sprintf_s(fVsPPq4_name, Nsavef1, "%sVsPPq4.txt", prefix1);
	sprintf_s(fVsTFq_name, Nsavef1, "%sVsTFq.txt", prefix1);
	sprintf_s(fPolEq_name, Nsavef1, "%sPolEq.txt", prefix1);
	sprintf_s(fPolHq_name, Nsavef1, "%sPolHq.txt", prefix1);

	fsaveVector(fq_name, q_array, Nq);
	fsaveVector(fVsq_name, Vsq_array, Nq);
	fsaveVector(fVsPPq1_name, VsPPq1_array, Nq);
	fsaveVector(fVsPPq4_name, VsPPq4_array, Nq);
	fsaveVector(fVsTFq_name, VsTFq_array, Nq);
	fsaveVector(fPolEq_name, PolEq_array, Nq);
	fsaveVector(fPolHq_name, PolHq_array, Nq);



	cout << "Calculating nonparabolic E(k) ...\n\n";
	double kkmax = (nkmax+2.0)*kF;
	kkmax = 5.0*kF;
	int Nkk = 5000;
	...
	*/
	...
	{
		double kk = kk_array[nkk];
		Eekk_array[nkk] = hbar*hbar*kk*kk/(2.0*effmE);
	}
...
*/	
	...
	double * EexPPkk_array = new double[Nkk];
	for (int nkk = 0; nkk < Nkk; nkk++)
	{
		double kk;
		kk = kk_array[nkk];
		...
		for (int nkp = 0; nkp < Nkp; nkp++)
		{
			...
			for (int nth = 0; nth < Ntheta; nth++)
			{
				...
				double theta = static_cast<double>(nth)*dtheta;
				double k_kp = sqrt(kk*kk+kp*kp-2.0*kk*kp*cos(theta));
				addintth = my_interp_eqdx(q_array,VsPPq1_array,Nq,k_kp)*dtheta;
				if (nth == 0 || nth == Ntheta-1)
					addintth *= 0.5;
				inttheta += addintth;
			}
		...
		EexPPkk_array[nkk] = intkp/pow(2.0*PI,2.0);
	}


	double * EexTFkk_array = new double[Nkk];
	for (int nkk = 0; nkk < Nkk; nkk++)
	{
		double kk;
		kk = kk_array[nkk];
		...
		for (int nkp = 0; nkp < Nkp; nkp++)
		{
			...
			for (int nth = 0; nth < Ntheta; nth++)
			{
				...
				double theta = static_cast<double>(nth)*dtheta;
				double k_kp = sqrt(kk*kk+kp*kp-2.0*kk*kp*cos(theta));
				addintth = my_interp_eqdx(q_array,VsTFq_array,Nq,k_kp)*dtheta;
				if (nth == 0 || nth == Ntheta-1)
					addintth *= 0.5;
				inttheta += addintth;
			}
		...
		EexTFkk_array[nkk] = intkp/pow(2.0*PI,2.0);
	}



	char fkkarray_name[Nsavef1];
	char fEexLH_name[Nsavef1];
	char fEexPP_name[Nsavef1];
	char fEexTF_name[Nsavef1];

	sprintf_s(fkkarray_name, Nsavef1, "%skk_array.txt", prefix1);
	sprintf_s(fEexLH_name, Nsavef1, "%sEexLH.txt", prefix1);
	sprintf_s(fEexPP_name, Nsavef1, "%sEexPP.txt", prefix1);
	sprintf_s(fEexTF_name, Nsavef1, "%sEexTF.txt", prefix1);

	fsaveVector(fkkarray_name, kk_array, Nkk);
	fsaveVector(fEexLH_name, Eexkk_array, Nkk);
	fsaveVector(fEexPP_name, EexPPkk_array, Nkk);
	fsaveVector(fEexTF_name, EexTFkk_array, Nkk);
	
	cout << "finished calculating exchange energy!\n";
	getchar();
	...
	for (int nk = 0; nk < Nk; nk++)
		delete [] Vskkp[nk];
	delete [] Vskkp, fFDE, fFDH, k_array;
...
*/
	...
	delete [] z_grid, Ve_grid, Ve_grid_halfdzshift, Vhh_grid, Vhh_grid_halfdzshift;

	delete [] layer, isbarrier, border;

	delete [] dipk_array;

	delete [] q_array, Vsq_array, kk_array, Eekk_array, Ehkk_array, Eexkk_array, EeRkk_array, EhRkk_array;

	for (int n = 0; n < nEe; n++)
		delete [] eigVe[n];
	delete [] eigEe, eigVe, effmxye_ave;

	for (int n = 0; n < nEhh; n++)
		delete [] eigVhh[n];
	delete [] eigEhh, eigEhhv, eigVhh, effmxyhh_ave;
